...
const transform: AxiosTransform = {
  ...
  beforeRequestHook: (config, options) => {
    const { apiUrl: optionApiUrl, joinPrefix, joinParamsToUrl, formatDate, joinTime = true } = options;

    if (joinPrefix) {
      config.url = `${urlPrefix}${config.url}`;
    }

    // 优先使用传入的apiUrl，否则使用环境变量中的apiUrl
    if (optionApiUrl && isString(optionApiUrl)) {
      config.url = `${optionApiUrl}${config.url}`;
    } else if (apiUrl) {
